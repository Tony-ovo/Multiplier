...
class SignedLow6Model(nn.Module):
    ...
    @torch.no_grad()
    def randomize_mutable_logits(
        self,
        mean: float = 0.0,
        std: float = 1.0,
        generator: torch.Generator | None = None,
    ) -> dict:
        """Randomize each distinct trainable LUT table and report provenance."""
        values: list[torch.Tensor] = []
        saved_values: dict[str, dict] = {}
        table_count = 0
        mutable_count = 0
        for module_name, module in self.core.named_modules():
            if not isinstance(module, TrainableLUT6):
                continue
            table_count += 1
            mutable_count += module.randomize_mutable_logits(
                mean=mean, std=std, generator=generator,
            )
            if bool(torch.any(module.mutable_mask)):
                mutable_indices = torch.nonzero(module.mutable_mask, as_tuple=False).flatten()
                mutable_values = module.logits[module.mutable_mask].detach()
                values.append(mutable_values)
                saved_values[module_name] = {
                    'bits': [int(v) for v in mutable_indices.cpu().tolist()],
                    'logits': [float(v) for v in mutable_values.cpu().tolist()],
                }
        if not values:
            raise ValueError('model has no mutable LUT logits')
        joined = torch.cat(values)
        if int(joined.numel()) != mutable_count:
            raise AssertionError('mutable-logit accounting mismatch')
        return {
            'distribution': 'normal',
            'requested_mean': float(mean),
            'requested_std': float(std),
            'trainable_lut_tables': table_count,
            'mutable_logit_count': mutable_count,
            'sample_mean': float(joined.mean().cpu()),
            'sample_std': float(joined.std(unbiased=False).cpu()),
            'sample_min': float(joined.min().cpu()),
            'sample_max': float(joined.max().cpu()),
            'positive_fraction': float((joined >= 0).to(torch.float32).mean().cpu()),
            'mutable_logits': saved_values,
        }
