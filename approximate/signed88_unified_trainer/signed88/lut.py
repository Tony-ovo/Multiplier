...
class TrainableLUT6(nn.Module):
    ...
    def randomize_mutable_logits(
        ...
        """Draw every trainable LUT logit directly from ``Normal(mean, std)``.

        Frozen INIT addresses are deliberately left untouched.  They are not
        trainable parameters of the hardware design and ``table()``/
        ``hard_bits()`` always substitute their fixed RTL values.
        """
        mean = float(mean)
        std = float(std)
        ...
        if count:
            samples = torch.empty(count, dtype=self.logits.dtype, device=self.logits.device)
            samples.normal_(mean=mean, std=std, generator=generator)
