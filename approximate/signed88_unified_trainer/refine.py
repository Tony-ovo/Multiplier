...
from signed88.common import (
    GEMM_OBJECTIVE_SCHEMA, ObjectiveWeights, hex_to_int, int_to_hex,
    is_gemm_objective, read_json, validate_objective_weights, write_json,
)
...
def parse_args():
    ...
    p.add_argument('--out-dir',default='refine_run')
    ...
    p.add_argument('--max-workload-nmse',type=float,default=-1)
    p.add_argument('--max-abs-workload-bias',type=float,default=-1)
    p.add_argument('--max-conditional-bias-rms',type=float,default=-1)
...
def valid(m,args):
    ...
    if args.max_workload_nmse>=0 and m.workload_NMSE>args.max_workload_nmse: return False
    if args.max_abs_workload_bias>=0 and abs(m.workload_bias)>args.max_abs_workload_bias: return False
    if args.max_conditional_bias_rms>=0 and max(m.workload_conditional_bias_a_rms,m.workload_conditional_bias_b_rms)>args.max_conditional_bias_rms: return False
    return True
...
def main():
    ...
    gemm_tagged=is_gemm_objective(inherited_obj,src.get('objective_schema'))
    if inherited_obj and not gemm_tagged:
        print('[objective] legacy artifact detected; using current GEMM NMSE/bias defaults (CLI overrides still apply)')
    ...
    protected=('best_signed88_inits.json','steps.json','best_rtl')
    existing=[name for name in protected if (out/name).exists()]
    if existing:
        raise FileExistsError(
            f'refine output directory already contains protected artifact(s): {existing}; '
            'use a new --out-dir'
        )
    out.mkdir(parents=True,exist_ok=True); rng=random.Random(args.seed); cache={}
    def eval_cached(x):
        k=key(x,design)
    ...
    def record(stage,flips_,old,new):
        row={'stage':stage,'flips':[f'{n}:{b}' for n,b in flips_],'old_score':old.objective_score,'new_score':new.objective_score,'metrics':new.to_dict()};steps.append(row);write_json(out/'steps.json',{'steps':steps});print(f'[{stage}] ACCEPT {row["flips"]} {old.objective_score:.10f}->{new.objective_score:.10f} wNMSE={new.workload_NMSE:.8g} wBias={new.workload_bias:+.5f}')
    ...
    artifact=design.artifact(inits,metrics=current.to_dict(),extra={'stage':'refined','base_json':str(Path(args.base_inits_json).resolve()),'calibration':profile.metadata(),'objective_schema':GEMM_OBJECTIVE_SCHEMA,'objective_weights':objective.__dict__,'refine_args':vars(args),'accepted_steps':steps})
    write_json(out/'best_signed88_inits.json',artifact);design.export_rtl(Path(args.rtl_template_root),out/'best_rtl',inits,metadata={'metrics':current.to_dict(),'calibration':profile.metadata(),'objective_schema':GEMM_OBJECTIVE_SCHEMA,'objective_weights':objective.__dict__})
